...
#     """Sorts images by timestamp and returns the start and end dates."""
#     if not dated_images:
...
#     """Identifies device switching events and generates usage insights."""
#     insights = []
#
#     if len(unique_cameras) > 1:
#         insights.append(f"Found {len(unique_cameras)} devices - Potential device switching detected.")
#
#     for i in range(1, len(dated_images)):
#         prev_cam, curr_cam = dated_images[i - 1].get("camera_model"), dated_images[i].get("camera_model")
#         if prev_cam and curr_cam and prev_cam != curr_cam:
#             msg = f"On {dated_images[i]['datetime']}, user switched from {prev_cam} to {curr_cam}."
#             if msg not in insights: insights.append(msg)
#
#     return insights
#
#
# def analyze(images_data):
#     """Orchestrates image data analysis and returns structured insights."""
#     if not images_data:
#         return {"error": "No data provided"}
#
#     # Filter data for GPS, valid dates, and unique camera models
#     images_with_gps = [img for img in images_data if img.get("has_gps")]
#     dated_images = [img for img in images_data if img.get("datetime")]
#     cameras = list(set([img.get("camera_model") for img in images_data if img.get("camera_model")]))
#
#     # Execute sub-analysis for dates and device behavior
#     date_range = get_date_range(dated_images)
#     insights = get_device_insights(dated_images, cameras)
#
#     if images_with_gps:
#         insights.append(f"Location data available for {len(images_with_gps)} images.")
#
#     return {
#         "total_images": len(images_data),
...
#         results = extractor.extract_all(folder_path)
#         print("\n--- Intelligence Analysis Results ---")
#         print(json.dumps(analyze(results), indent=4))
#     else:
...
#     """Analyzes patterns like active hours and activity bursts."""
#     if not dated_images:
#         return []
#
#     behavior_insights = []
#
#     # Pattern 1: Active Hours (Night vs Day)
...
# def analyze(images_data):
#     """Orchestrates image data analysis and returns structured insights."""
#     # Handling empty data to prevent crashes
#     if not images_data:
#         return {"error": "לא התקבלו נתונים לניתוח", "insights": ["אין נתונים זמינים"]}
#
#     # Filter data for GPS, valid dates, and unique camera models
#     images_with_gps = [img for img in images_data if img.get("has_gps")]
#     dated_images = [img for img in images_data if img.get("datetime")]
...
#
#     if folder_path.exists():
#         # Safely extract and analyze
#         results = extractor.extract_all(folder_path) or []
